...
import com.zzw.socketdemo.base.BaseApplication;
...
public class SPUtil {
    ...
    public static SPUtil getInstance(String spName) {
        if (isSpace(spName)) spName = "guangLan";
     ...
     * @return 读取结果
     */
    public <T> T getSerializable(@NonNull String key, @Nullable T defaultValue) {
        ...
        if (data != null) {
            try {
                ...
                @SuppressWarnings("unchecked") T result = (T) readObject;
                return result;
            } catch (Exception e) {
                e.printStackTrace();
            }
